...
export function UpdateToast() {
  ...
  // 토스트가 보이면 일정 시간 후 자동으로 닫는다. 업데이트 진행 중에는 유지한다.
  useEffect(() => {
    if (!waitingWorker || dismissed || activating) return;
    const timerId = window.setTimeout(dismissToast, AUTO_DISMISS_MS);
    return () => window.clearTimeout(timerId);
  }, [waitingWorker, dismissed, activating]);
